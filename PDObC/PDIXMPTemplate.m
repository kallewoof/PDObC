...
+ (id)templateForXMPArchive:(PDIXMPArchive *)archive
{
    ...
    PDIXMPLicense license = [self licenseForXMPArchive:archive extras:extras];
    if (license == PDIXMPLicenseUndefined) license = PDIXMPLicenseCommercial;
    
    PDIXMPTemplate *template = [self templateForLicense:license];
    if (extras[@"mvs"]) template.licenseMajorVersionString = extras[@"mvs"];
    if (extras[@"morePerms"]) template.morePermissions = extras[@"morePerms"];
    if (extras[@"attributionName"]) template.attributionName = extras[@"attributionName"];
    if (extras[@"attributionURL"]) template.attributionURL = extras[@"attributionURL"];
    if (extras[@"useGuidelinesURL"]) template.useGuidelinesURL = extras[@"useGuidelinesURL"];
    
    [archive selectRoot];
    [archive selectElement:@"x:xmpmeta"];
    [archive selectElement:@"rdf:RDF"];
    NSString *rights = [[archive findElements:@"rdf:Description"] firstAvailableElementValueForPath:@[@"dc:rights", @"*", @"rdf:li"]];
...
+ (PDIXMPLicense)licenseForXMPArchive:(PDIXMPArchive *)archive extras:(NSMutableDictionary *)extras
{
    if (licenseUrls == nil) PDIXMPTemplateSetup();
    
    __block PDIXMPLicense resultingLicense = PDIXMPLicenseUndefined;
    
    [archive if:@"/x:xmpmeta/rdf:RDF" do:^{
    //    [archive selectRoot];
    //    [archive selectElement:@"x:xmpmeta"];
    //    [archive selectElement:@"rdf:RDF"];
        
        [archive if:@"rdf:Description[xmlns:cc=http://creativecommons.org/ns#]" do:^{
        
//        if ([archive selectElement:@"rdf:Description" withAttributes:@{@"xmlns:cc": @"http://creativecommons.org/ns#"}]) {
            if (extras) {
                [archive if:@"cc:morePermissions" do:^{
//                if ([archive selectElement:@"cc:morePermissions"]) {
                    // the more permissions expression is a URL pointing to a place where permissions beyond the active license may be obtained
                    NSString *value = [archive stringForAttribute:@"rdf:resource"];
                    if (value) extras[@"morePerms"] = value;
//                    [archive selectParent];
//                }
                }];
                [archive if:@"cc:attributeName" do:^{
//                if ([archive selectElement:@"cc:attributionName"]) {
                    NSString *value = [archive stringForAttribute:@"rdf:resource"];
                    if (value) extras[@"attributionURL"] = value;
                    value = [archive elementContent];
                    if (value) extras[@"attributionName"] = value;
//                    [archive selectParent];
//                }
                }];
            }
            
            [archive if:@"cc:license" do:^{
                ...
                if (url) {
                    ...
                    if ([url hasSuffix:@"/legalcode"]) {
                        ...
                        if ([@"zero" isEqualToString:url.stringByDeletingLastPathComponent.lastPathComponent]) {
                            resultingLicense = PDIXMPLicenseCC0;
                            return;
                        }
                    ...
                    if (index != NSNotFound) {
                        ...
                        if (extras && !extras[@"mvs"]) extras[@"mvs"] = [self defaultMajorVersionStringForLicense:(PDIXMPLicense)index];
                        resultingLicense = (PDIXMPLicense) index;
                        return;
                    }
                }
            }];
...
//            [archive selectParent];
        ...
        if (resultingLicense != PDIXMPLicenseUndefined) return;

        [archive if:@"rdf:Description" do:^{
//        if ([archive selectElement:@"rdf:Description"]) {
            // look for a dc:rights entry
            [archive if:@"dc:rights/*/rdf:li" do:^{
//            if ([archive selectElement:@"dc:rights"] && [archive selectElement:@"rdf:Alt"] && [archive selectElement:@"rdf:li"]) {
                ...
                if ([rights rangeOfString:@"all rights reserved" options:NSCaseInsensitiveSearch].location != NSNotFound) {
                    if (extras) extras[@"mvs"] = nil;
                    resultingLicense = PDIXMPLicenseCommercial;
                    return;
                }
            }];
//            }
            ...
            for (NSInteger i = 0; i < licenseNames.count; i++) {
                if ([string hasPrefix:licenseNames[i]]) {
                    if (extras) {
                        ...
                        extras[@"mvs"] = [self defaultMajorVersionStringForLicense:(PDIXMPLicense)i];
                    ...
                    resultingLicense = (PDIXMPLicense) i;
                    return;
                }
            }
        }
...
- (void)applyToArchive:(PDIXMPArchive *)archive withAuthorName:(NSString *)authorName extra:(NSDictionary *)extraDict
{
    NSMutableDictionary *extra = extraDict ? extraDict.mutableCopy : [NSMutableDictionary dictionary];
    ...
    [archive in:@"/x:xmpmeta/rdf:RDF" do:^{
        //    [archive selectRoot];
        ...
        if (_license != PDIXMPLicenseCommercial) {
            [archive in:@"rdf:Description[rdf:about=&xmlns:xapRights=http://ns.adobe.com/xap/1.0/rights/]/xapRights:Marked" do:^{
...
//        NSMutableDictionary *d = extra ? extra.mutableCopy : [NSMutableDictionary dictionary];
        ...
        if (! extra[@"xmp:ModifyDate"])     extra[@"xmp:ModifyDate"] = datetimeString;
        if (! extra[@"dc:rights"])          extra[@"dc:rights"] = _rights ? _rights : [PDIXMPTemplate defaultRightsForLicense:_license major:_licenseMajorVersionString withAuthor:authorName];
        if (! extra[@"xmp:CreatorTool"])    extra[@"xmp:CreatorTool"] = [NSString stringWithFormat:@"Pajdeg Ob-C (Pajdeg v. " PAJDEG_VERSION ")"];
        ...
        [archive in:@"rdf:Description" attributes:@[nslist] do:^{
...
//        } [archive selectParent];
        ...
        if (ccLicense[_license]) {
            [archive in:@"rdf:Description[rdf:about=&xmlns:cc=http://creativecommons.org/ns#]" do:^{
//            [archive createElement:@"rdf:Description" withAttributes:@{@"rdf:about":       @"",
//                                                                       @"xmlns:cc":        @"http://creativecommons.org/ns#"}]; {
                [archive in:[NSString stringWithFormat:@"cc:license[rdf:resource=%@]", [self resolvedLicenseURL]] do:nil];
//                [archive createElement:@"cc:license" withAttributes:@{@"rdf:resource":     [self resolvedLicenseURL]}];
//                [archive selectParent];
                if (_morePermissions) {
                    [archive in:[NSString stringWithFormat:@"cc:morePermissions[rdf:resource=%@]", _morePermissions] do:nil];
//                    [archive createElement:@"cc:morePermissions" withAttributes:@{@"rdf:resource": _morePermissions}];
//                    [archive selectParent];
                }
                if (_attributionName) {
                    NSDictionary *attrib = nil;
                    if (_attributionURL) attrib = @{@"rdf:resource": _attributionURL};
                    [archive in:@"cc:attributionName" attributes:@[attrib] do:^{
//                    [archive createElement:@"cc:attributionName" withAttributes:attrib];
                        [archive setElementContent:_attributionName];
                    }];
//                    [archive selectParent];
                }
                if (_useGuidelinesURL) {
                    [archive in:[NSString stringWithFormat:@"cc:useGuidelines[rdf:resource=%@]", _useGuidelinesURL] do:nil];
//                    [archive createElement:@"cc:useGuidelines" withAttributes:@{@"rdf:resource": _useGuidelinesURL}];
//                    [archive selectParent];
                }
            }];
//            } [archive selectParent];
        ...
        [NSException raise:@"PDIXMPArchiveException" format:@"x:xmpmeta/rdf:RDF path not found"];
    }];
